...
const emptyForm = { articleId: '', quantite: 1, provenance: '', document: '', observation: '' }
...
export default function StockEntrees() {
  ...
  const entrees = mouvements.filter((m) => m.type === 'entree').sort((a, b) => (a.date < b.date ? 1 : -1))
  ...
  async function handleSubmit(e) {
    ...
    if (result.ok) {
      ...
      setFeedback(`Stock de ${article.designation} mis à jour : ${article.stock} + ${form.quantite}.`)
      setForm(emptyForm)
    }
  }
  ...
  return (
    <div>
      <PageHeader title="Entrées de stock" description="Enregistrez un nouvel approvisionnement et mettez à jour le stock automatiquement." />

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
        <form onSubmit={handleSubmit} className="lg:col-span-2 h-fit rounded-lg border border-line bg-surface p-4">
          <p className="mb-3 text-sm font-medium text-ink">Nouvelle entrée</p>
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-muted">Quantité reçue</label>
              <input
                type="number"
                min={1}
                required
            ...
            <div>
              <label className="mb-1 block text-xs text-muted">Fournisseur ou provenance</label>
              <input
            ...
            <Button type="submit" variant="gold" icon={ArrowDownToLine} className="w-full">
        ...
        <div className="lg:col-span-3 overflow-hidden rounded-lg border border-line bg-surface">
          <div className="border-b border-line px-4 py-2.5">
            <p className="text-sm font-medium text-ink">Historique des entrées</p>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody>
              {entrees.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-sm text-muted">Aucune entrée enregistrée.</td>
                </tr>
              ) : entrees.map((m) => (
